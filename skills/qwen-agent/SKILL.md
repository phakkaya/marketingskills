...
- **This is the default invocation.** The flag list scopes which tools the subagent may use without a prompt, so it can finish a menial job unattended.
- The alias bakes in `--allowedTools '*'`, which Claude Code **silently ignores** with a warning. That warning is expected and harmless — the `--allowedTools` you append is what takes effect.
- For edit-only, lower-risk tasks you may instead use `--permission-mode acceptEdits` (auto-accepts file edits, but Bash still prompts).
...
Qwen runs with a **128k-token context window** — much smaller than Claude's. The whole job (your prompt + every file it reads + its own reasoning and edits) has to fit inside it.

- **Estimate the footprint** before delegating: roughly the bytes of files it must read ÷ 4 ≈ tokens. If a single task would pull in large files, it won't fit.
- **Break large jobs into independent chunks** that each touch a bounded slice. Run the chunks as separate `claude-9arm` invocations.
- **Don't make it read what it doesn't need.** Point it at the exact files/paths required; never tell it to "scan the repo."
...
When a job is inherently too big to slice cleanly (it needs whole-codebase context), that's a sign it isn't a qwen task — keep it yourself.
...
- **Background / parallel (run several at once):** redirect to a log and run with `run_in_background: true`, then read the log when it finishes:
...
1. Confirm the task is menial and low-risk. If it needs design judgment or this chat's context, **do it yourself** — don't delegate.
2. Check it fits qwen's **128k context window** — estimate the file footprint and split large jobs into bounded chunks.
...
To stop per-call permission prompts on delegated runs, add a Bash allow rule for the command:
